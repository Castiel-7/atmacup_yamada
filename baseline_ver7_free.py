...
各カラムが原子的な値（つまり、分割できない値）のみを持つようにテーブルが設計されていること。
同じテーブル内で重複した行が存在しないこと。
例: 住所カラムが「都道府県」と「市区町村」に分割される。
第二正規形 (2NF):

1NFのすべての条件を満たしていること。
テーブルのすべての非キーカラムが、そのテーブルの主キー全体に完全に機能的に依存していること。
部分的な依存関係が排除されていること。
例: 顧客の住所情報と注文情報が同じテーブルに存在する場合、それらを分割して顧客テーブルと注文テーブルの2つのテーブルにする。
第三正規形 (3NF):

2NFのすべての条件を満たしていること。
非キーカラムの間にトランジティブな依存関係（間接的な依存関係）が存在しないこと。
例: 顧客テーブルに「都道府県」と「都道府県の地域（例: 関東、関西など）」のカラムが存在する場合、それらを分割して都道府県テーブルを別に作成する。
